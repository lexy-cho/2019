...
+function ($) {
	...
	DUDatePicker.prototype = {
		...
		setupCalendar: function () {
			...
			var D_NAME_SET = [];
			if(options.locale === 'kr') D_NAME_SET = DAY_NAME_LOCALE.KR;
			else D_NAME_SET = DAY_NAME_LOCALE.EN;
			
			var inView = {
				wrapper: $('<div class="dudp__calendar"></div>'),
				header: $('<div class="dudp__cal-month-year"></div>'),
				weekDays: $('<div class="dudp__weekdays"><span>'+D_NAME_SET[0]+'</span><span>'+D_NAME_SET[1]+'</span><span>'+D_NAME_SET[2]+'</span><span>'+D_NAME_SET[3]+'</span><span>'+D_NAME_SET[4]+'</span><span>'+D_NAME_SET[5]+'</span><span>'+D_NAME_SET[6]+'</span></div>'),
	...
	$.fn.duDatepicker.defaults = {
		...
		locale: 'kr'				// Determines the Language of letters on Calendar
	};
